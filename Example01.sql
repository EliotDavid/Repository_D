...
CREATE TABLE Reservation3(
    ...
    # CASCADE 옵션 : 참조하는 데이터가 변경되거나 삭제되면 참조테이블에서도 데이터가 삭제와 수정이 같이 이루어짐
    
    # 참조키 만드는 방법은 아래와 같음 (그리고 얘에 대한 ON UPDATE의 CASCADE를 쓰는 방법)
    CONSTRAINT reservation3_FK FOREIGN KEY(roomNumber)
	REFERENCES Room(roomNumber) 
    # ON UPDATE CASCADE # 변경되면 얘도 같이 변경되라라고 제약조건을 걸어주는 코드 
    # ON DELETE CASCADE # 삭제되면 얘도 같이 삭제되라라고 제약조건을 걸어주는 코드
	...
	# ON UPDATE / DELETE : 업데이트 될 때 / 영어처럼 삭제 될 때라고 해석하면 됨
    # ON UPDATE NO ACTION 
    # ON DELETE NO ACTION
    ...
    # RESTRICT 옵션 : 참조하는 데이터가 변경되거나 삭제되거나 삭제가 불가능
    # 기본값 
    ON UPDATE RESTRICT
    ON DELETE RESTRICT
...
VALUES('이영희', '2023-01-24', null); # 위에 Reservation3테이블을 만들고 난 다음에 이 코드를 바로 실행했을 때 안 되는 이유는 roomNumber키를 참조키로 걸어뒀는데 얘가 참조하고 있는 Room테이블의 roomNumber를 지우는 코드를 밑에서 한 번 날려서 roomNumber라는 필드가 없어서 참조하고 있는 테이블에서 참조되는 테이블로 값을 수셔넣으려고 해도 안 들어가져서 에러가 나는거임(참조키 특성과 참조키일 때 값을 대입해주려고 할 때 주의해야될 상황 또는 조건 확인하기)  
 ...
 # ROOM테이블을 수정해서 ROOMNUMBER가 2901호인애를 2902호인애로 바꿀거임
 UPDATE Room SET roomNumber = 2902 WHERE roomNumber = 2901; # 이 녀석을 변경하면 Reservation3에 2901호로 등록되어있던 애도 2902호로 바뀜
 
 UPDATE Room SET roomNumber = 2903 WHERE roomNumber = 2902; # 이것도 막힘
 
 DELETE FROM Room WHERE roomNumber = 2902; # 이렇게 Room테이블에서 2902호로 등록되어있던 애를 지우면 2902호 칼럼을 가지고 있던 얘를 참조하고 있는 


# JOIN 
# JOIN은 여러 테이블을 조합하여 하나의 테이블로 표현하는 방법임
# 일반적으로 SELECT구문에 사용됨 

# * INNER JOIN
# ON절에 조건을 만족하는 데이터만 가져옴 
# SELECT 칼럼명 FROM 테이블1명 INNER JOIN 테이블2명 ON 조인조건

SELECT * FROM Reservation3 INNER JOIN Room ON Reservation3.roomNumber = Room.roomNumber; # ON절로 Reservation3의 roomNumber와 Room의 roomNumber가 같은 애만 가져와라는 조건을 붙여줬음

# MySQL에서는 INNER JOIN 구문을 ,로 대체하고 ON을 WHERE로 대체해서 사용가능
...
#SELECT id, name, reservationDate, note, roomNumber, roomSize, roomName
#FROM Reservation3 AS RV, Room AS RM
#WHERE RV.roomNumber = RM.roomNumber; # 이렇게하면 실행이 안되는데 그 이유는 roomNumber 지금 2개가 있는데 이러면 쿼리가 roomNumber을 Reservation3에서 가져와야되는지 Room에서 가져와야되는지 모름 

# 그래서 아래처럼 정확히 어떤 걸 뽑아줘라라고 적어줘야함
# 두 테이블 이상을 FROM절에서 사용할 때는 선택할 컬럼명 앞에 어떤 테이블의 컬럼인지를 직접 지정해주는 것이 좋음 
...
#SELECT * FROM Reservation3 AS RV, Room AS RM
#WHERE RV.roomNumber = RM.roomNumber;

# LEFT JOIN
# 왼쪽 테이블에 참조키를 기준으로 조인결과를 나열함

# SELECT 칼럼명 FROM 테이블1명 LEFT JOIN 테이블2명 ON 조인조건 [WHERE 조건]
SELECT * FROM  Reservation3 AS RV LEFT JOIN Room AS RM ON RV.roomNumber = RM.roomNumber; # RV의 roomNumber와 RM의 roomNumber가 동일한 애를 출력
# 이영희의 roomNumber는 null이지만 왼쪽테이블인 Reservation3의 roomNumber3가 참조키라서 이 참조키를 기준으로 보여주는거라서 Room테이블의 roomNumber가 null이어도 보여줌

SELECT * FROM Reservation3;
# RIGHT JOIN
# 오른쪽 테이블에 참조키를 기준으로 조인 결과를 나열(꼭 참조키일 필요는 없음 참조키 자리에 값을 넣기만 넣어도 그 테이블의 값을 기준으로 보여줌) 
# SELECT 칼럼명 FROM 테이블1명 LEFT JOIN 테이블2명 ON 조인조건 [WHERE 조건]
SELECT * FROM Reservation3 AS RV RIGHT JOIN Room AS RM ON RV.roomNumber = RM.roomNumber;


# 서브쿼리 
# 테이블의 검색결과를 조건으로 사용하거나 FROOM절에서 새로운 테이블로 사용할 수 있도록 한 것

# WHERE절에서 사용하는 방법 
# SELECT 컬럼명 FROM 테이블A 
# WHERE 컬럼명 = (SELECT 컬럼명 FROM 테이블B명 WHERE 조건) 
# 또는
# WHERE 칼럼명 IN (SELECT 칼럼명 FROM 테이블B명 WHERE 조건)
# WHERE 조건에서 쓸 때는 주의해야될 점이 있음 
# WHERE 조건에서는 하나 또는 0개의 결과가 나와야 하는데 그 때는 주키나 유니크키 같은거로 걸어주는게 좋다고함
# IN 연산자를 쓸 때는 IN이 1 : 다 관계여서 SELECT 칼럼명이 1 이 되어야되고 WHERE 조건이 다 관계가 되어야 된다라는건가?


# = 연산자를 쓴 서브쿼리 ( 1 : 1 관계라서 값을 한 개만 넣음)
SELECT * FROM Reservation3
WHERE roomNumber = (
	SELECT roomNumber 
    FROM Room
...
# IN으로 받았기 때문에 0개 이상이라서 된다고 함 ( 뭐가 된다는거지)
SELECT * FROM Reservation3
WHERE roomNumber IN (
	SELECT roomNumber 
    FROM Room
);    # Room 테이블에서 Reservation 3의 roomNumber의 칼럼값과 같은게 있다면 그걸 도출해라 이 말인듯
	   # IN은 배열이니까 
...
SELECT RV.iv, RV.name, RV.reservationDate, RV.note, RM.roomNumber, RM.roomSize, RM.roomName
From Reservation3 AS RV, Room AS RM
WHERE RV.roomNumber = RM.roomNumber;
